...
<script>
  export default {
    ...
    computed: { //  监听选中值
      history () { // 买入深度
        return [{name: this.$t('tableheder.amount'), val: 'amount'}, {name: this.$t('tableheder.price'), val: 'price', class: 'gree'}]
      },
      history1 () { // 卖出深度
        return [{name: this.$t('tableheder.price'), val: 'price', class: 'red'}, {name: this.$t('tableheder.amount'), val: 'amount'}]
      },
      tableTr () {
        let tableHeader = []
