  ...
  <el-container>
    <el-aside width="4rem">
      ...
      <div class="small-box skin-bg" style="min-height: 3.05rem">
        ...
        <span v-if="$store.state.sopttrading.myBalance.length" class=" btn-color update-btn" @click="updateClick">{{$t("btnname.update")}}</span>
...
export default {
    ...
    watch: {// 交易市场改变，货币改变清空表单。
      navBazzer (n, o) {
        ...
        this.currenty({siteId: Bazzer_SiteId}, n)// 动态加载货币
        this.balancePost({siteId: Bazzer_SiteId, size: 5})// 个人资产
      ...
      sellPriceType () {
        this.balancePost({siteId: this.navBazzer.id, size: 5})// 个人资产
      }
    },
    components: {
    ...
    created () {
      ...
      if (currenty_Obj) {
        ...
        this.currenty({siteId: currenty_Obj}, this.navBazzer)// 动态加载货币
        this.balancePost({siteId: currenty_Obj, size: 5})// 个人资产
      }
    },
    methods: {
      ...
      currenty (data, Obj) {
        if (Obj.currentyList) {
          this.currentySelect(Obj.currentyList)
          this.$store.dispatch('currenty', Obj.currentyList)
          return
        }
        this.$postAxios.currentyAxios(data).then((res) => {
          const data_Obj = res.data
          if (data_Obj.code == 200) {
            this.currentySelect(data_Obj.data)// 首页快速选择币对，选择币对
            Obj.currentyList = data_Obj.data // 添加到交易所里面
      ...
      balancePost (data) {
        this.$postAxios.balance(data).then((res) => {
          ...
          if (data_Obj.code == 200) {
            ...
            this.$store.dispatch('myBalance', data_Obj.data.records)
            return
          }
      ...
      updateClick () { // 更新资产
        this.$postAxios.updateBalance({siteId: this.navBazzer.siteId}).then((ref) => {
          if (ref.data.code == 200) {
            this.$store.dispatch('pricesSet', !this.$store.state.sopttrading.pricesSet)// 可用价格刷新
            return
          }
